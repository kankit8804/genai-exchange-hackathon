...
function ViewAllPageInner() {
  ...
  const fetchData = async (projectId: string, skipCheck: boolean = false) => {
    try {
      ...
      if (list.length > 0)
        showNotification(
          `Loaded ${list.length} previously generated test case${
            list.length > 1 ? "s" : ""
          }.`
        );
      setTestCases(list);
  ...
  const handleDragEnd = (result: any) => {
    ...
    if (
      source.droppableId === "pushed" &&
      destination.droppableId === "generated"
    ) {
    ...
    const updated = testCases.map((tc) => {
      ...
      if (destination.droppableId === "generated")
  ...
  const handleSaveTestCase = async (data: any) => {
    if (!selectedProject) {
      showNotification(
        "Please select a project before adding a test case.",
        true
      );
      return;
    }
    try {
      ...
      const response = await post<{
        ...
        req_id: string;
        createdAt: string;
      }>(`${API_BASE}/manual/testcase`, payload);
      if (response?.test_id) {
      ...
      console.error(err);
      showNotification(
        "Failed to create test case. Please check your data and try again.",
        true
      );
      setAddingStatus("error");
    } finally {
  ...
  const handleMovedToPushed = (updatedTc: TestCase) => {
    setTestCases((prev) =>
      prev.map((t) =>
        t.test_id === updatedTc.test_id
          ? { ...t, ...updatedTc, isPushed: true, isRemoved: false }
          : t
      )
    );
  };
  ...
  const { generated, pushed, removed } = useMemo(() => {
    ...
    return {
      generated: testCases
        .filter((t) => !t.isPushed && !t.isRemoved && match(t))
        .sort(sortBySeverity),
      pushed: testCases
        .filter((t) => t.isPushed && !t.isRemoved && match(t))
        .sort(sortBySeverity),
      removed: testCases
        .filter((t) => t.isRemoved && match(t))
        .sort(sortBySeverity),
    };
                ...
                >
                  {(dragProvided, snapshot) => (
                    ...
                    >
                      <div
                        className={
                          snapshot.isDragging ? "bg-white rounded-md" : ""
                        }
                      >
                        <ResultItem
                          ref={(el) => {
                          ...
                          onUpdated={() => {
                            if (selectedProject)
                              fetchData(selectedProject, true);
                          }}
