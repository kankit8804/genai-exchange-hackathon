...
function ViewAllPageInner() {
    ...
    const fetchProjects = async () => {
        ...
        try {
            ...
            const q = query(projectsRef, where("uid", "==", user.uid), orderBy("createdAt", "desc"));
            ...
            const data: Project[] = snapshot.docs.map((doc) => {
                ...
                return {
                    ...
                    createdAt: project.createdAt ? new Date(project.createdAt.seconds * 1000).toLocaleString() : "N/A",
                };
            });
            setProjects(data);
    ...
    const fetchData = async (projectId: string, skipCheck: boolean = false) => {
        try {
            ...
            if (!skipCheck && currentProjectId && currentProjectId === projectId) return;
            setTestCases([]);
            ...
            const list = Array.isArray(existing)
                ...
                ? existing.test_cases
                : [];
            if (list.length > 0) showNotification(`Loaded ${list.length} previously generated test case${list.length > 1 ? "s" : ""}.`);
            setTestCases(list);
        } catch (err) {
    ...
    useEffect(() => {
        if (isFromDashboard && testCases.length > 0) {
            ...
            if (firstProjectId && selectedProject !== firstProjectId) setSelectedProject(firstProjectId);
        }
    ...
    const handleDragEnd = (result: any) => {
        ...
        if (source.droppableId === "pushed" && destination.droppableId === "generated") {
            showNotification("Can't move pushed test cases back to Generated.", true);
            return;
        }
        ...
        const updated = testCases.map((tc) => {
            ...
            if (destination.droppableId === "pushed") return { ...tc, isPushed: true, isRemoved: false };
            if (destination.droppableId === "removed") return { ...tc, isRemoved: true, isPushed: false };
            if (destination.droppableId === "generated") return { ...tc, isRemoved: false, isPushed: false };
            return tc;
        });
        ...
        if (destination.droppableId === "pushed") {
            ...
            const currentTc = testCases.find((tc) => tc.test_id === draggableId);
            if (ref && ref.triggerPush && !currentTc?.isPushed) ref.triggerPush();
        }
    };
    ...
    const handleSaveTestCase = async (data: any) => {
        ...
        try {
            ...
            const response = await post<{ test_id: string; req_id: string; createdAt: string }>(
                `${API_BASE}/manual/testcase`,
                payload
            );
            if (response?.test_id) {
                ...
                setTestCases((prev) => [
                    ...prev,
                    { ...data, test_id: response.test_id, req_id: response.req_id, project_id: selectedProject, isPushed: false, isRemoved: false, createdAt: response.createdAt },
                ]);
    ...
    const handleMovedToPushed = (updatedTc: TestCase) => {
        setTestCases((prev) => prev.map((t) => (t.test_id === updatedTc.test_id ? { ...t, ...updatedTc, isPushed: true, isRemoved: false } : t)));
    };

    const { generated, pushed, removed } = useMemo(() => {
        const q = searchText.toLowerCase();
        const match = (t: TestCase) => !searchText || t.title.toLowerCase().includes(q) || t.test_id.toLowerCase().includes(q) || t.expected_result.toLowerCase().includes(q);
        const sortBySeverity = (a: TestCase, b: TestCase) => severityOrder.indexOf(a.severity.toLowerCase()) - severityOrder.indexOf(b.severity.toLowerCase());
        return {
        ...
        <div>
            <h2 className="text-lg font-semibold mb-3 flex items-center justify-between">
                <div className="flex items-center gap-2">
                    ...
                    {cases.length > 0 && <span className="text-sm font-medium text-slate-500">({cases.length})</span>}
                </div>
                {title === "Generated" && (
                    <button
                        onClick={() => setShowModal(true)}
                        title="Add Test Case"
                        className="mr-[5px] flex items-center justify-center w-7 h-7 rounded-md bg-emerald-100/70 hover:bg-emerald-200 transition-colors shadow-sm"
                    >
            ...
            <Droppable droppableId={droppableId}>
                {(provided) => (
                    <div ref={provided.innerRef} {...provided.droppableProps} className="flex-1 min-h-[200px] space-y-3 overflow-y-auto">
                        {cases.length ? (
                            cases.map((tc, index) => (
                                <Draggable key={tc.test_id} draggableId={tc.test_id} index={index}>
                                    {(dragProvided, snapshot) => (
                                        <div
                                            ...
                                            className={`transition-all duration-150 ${snapshot.isDragging ? "scale-[1.02] shadow-2xl cursor-grabbing" : "cursor-grab"}`}
                                        >
                                            <div className={snapshot.isDragging ? "bg-white rounded-md" : ""}>
                                                <ResultItem
                                                    ...
                                                    onMovedToPushed={handleMovedToPushed}
                                                />
                                            </div>
                                        </div>
                                    )}
            ...
            {showAdd && (
                <>
                    ...
                    <AddTestCaseModal open={showModal} onClose={() => setShowModal(false)} onSave={handleSaveTestCase} />
                </>
            )}
        </div>
    );
        ...
        <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 p-8 text-slate-800 flex flex-col">
            <div className="w-full max-w-7xl mx-auto flex flex-col flex-1">
                <div className="flex justify-between items-center mb-8">
                    <TestcaseHeader projects={projects} selectedProject={selectedProject} setSelectedProject={setSelectedProject} />
                    <div className="flex items-center gap-2 flex-1 max-w-md ml-auto">
                        <input type="text" value={searchText} onChange={(e) => setSearchText(e.target.value)} placeholder="Search test cases..." className="flex-1 rounded-md border border-black/30 bg-white px-3 py-2 text-sm text-black placeholder-slate-400 focus:outline-none focus:border-black" />
                        {searchText && (
                            <button onClick={() => setSearchText("")} className="text-sm font-medium text-black hover:text-gray-600">
                                Clear
                            </button>
                        )}
                    </div>
                    <button onClick={() => router.back()} className="ml-4 rounded-md bg-emerald-600 px-4 py-2 text-sm font-medium text-white hover:bg-emerald-700">
                        ← Back
                    </button>
                </div>
